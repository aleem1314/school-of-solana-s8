...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    let reaction_acc = &mut ctx.accounts.tweet_reaction;


    match reaction {
        ReactionType::Like => tweet.likes = tweet.likes.checked_add(1).unwrap(),
        ReactionType::Dislike => tweet.dislikes = tweet.dislikes.checked_add(1).unwrap(),
    }

    reaction_acc.reaction_author = ctx.accounts.reaction_author.key();
    reaction_acc.parent_tweet = tweet.key();
    reaction_acc.reaction = reaction;
    reaction_acc.bump = ctx.bumps.tweet_reaction;
