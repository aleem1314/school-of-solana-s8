...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        ...
        if let Some(result) = op_type.perform(x, y) {
            let op = Operation::new(x, y, op_type);
        ...
        for i in 0..self.history.len() {
            ...
            if let Some(value) = result {
                output.push_str(&format!(
                    "{}: {} {} {} = {}\n",
                    i, op.first_num, sign, op.second_num, value
                ));
            }
        }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64> {
        ...
        let op = self.history[operation_index].clone();
